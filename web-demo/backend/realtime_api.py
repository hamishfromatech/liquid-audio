...
class RealtimeConversation:
    ...
    def __init__(self, system_prompt: str = ""):
        ...
        self.system_prompt = (
            system_prompt
            or "You are a helpful, friendly assistant. Keep responses concise and natural."
        )
    ...
    async def process_audio_stream(
        self,
        websocket: WebSocket,
        temp: float | int | None = 1.0,
        topk: float | int | None = 4,
    ):
                ...
                if chunk is not None:
                    ...
                    if len(audio_chunks) >= min_chunks:
                        await self._generate_response(
                            websocket, audio_chunks, temperature, top_k
                        )
                        audio_chunks = []
    ...
    async def _generate_response(
        self, websocket: WebSocket, audio_chunks: list, temp: float, topk: int | None
    ):
        ...
        try:
            ...
            audio_tensor = torch.tensor(
                audio_data.astype(np.float32) / 32_768.0, dtype=torch.float
            ).unsqueeze(0)
