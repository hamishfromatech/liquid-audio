...
class ConversationState:
    ...
    def __init__(self, system_prompt: str = ""):
        ...
        self.system_prompt = (
            system_prompt
            or "Respond with interleaved text and audio. You are a helpful, friendly assistant. Keep responses concise and natural."
        )
    ...
    def add_audio_input(self, audio_data: np.ndarray, sample_rate: int = 24000):
        ...
        audio_tensor = torch.tensor(audio_float, dtype=torch.float).unsqueeze(
            0
        )  # Add batch dimension
...
def generate_response(
    ...
    return (
        ...
        out_audio,
        out_modality,
        assistant_text.removesuffix("<|text_end|>").strip(),
    )
    ...
    try:
        while True:
            ...
            if message_type == "init":
                ...
                await websocket.send_json({"type": "init_response", "status": "ready"})

            # Handle audio input
            elif message_type == "audio":
                if not conversation:
                    await websocket.send_json(
                        {"type": "error", "message": "Conversation not initialized"}
                    )
                    continue
                ...
                try:
                    ...
                    await websocket.send_json(
                        {"type": "error", "message": f"Invalid audio data: {e!s}"}
                    )
                    continue
                ...
                # Generate response
                try:
                    out_text, out_audio, out_modality, text_response = (
                        generate_response(
                            conversation.chat,
                    ...
                    if out_text or out_audio:
                        conversation.chat.append(
                            ...
                            audio_out=(
                                ...
                                if out_audio
                                else torch.empty((8, 0), device=device)
                            ),
                            modality_flag=(
                                torch.tensor(
                                    [m.value for m in out_modality], device=device
                                ).unsqueeze(0)
                                if out_modality
                                else torch.empty((1, 0), device=device)
                            ),
            ...
            # Reset conversation
                ...
                await websocket.send_json({"type": "reset_response", "status": "reset"})
